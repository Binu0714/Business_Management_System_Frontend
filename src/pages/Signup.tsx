...
const Signup = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Registering new user for Binu Products...", formData);
  };

  return (
    <AuthLayout title="" subtitle="">
     
      <div className="bg-white/90 backdrop-blur-sm p-12 rounded-[50px] shadow-[0_25px_70px_rgba(0,0,0,0.06)] border border-white w-full max-w-[480px] mx-auto">
        
        {/* HEADING: Matches Login Page Typography */}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* BUTTON: Solid Vibrant Orange to match Login button */}
          <button 
            type="submit" 
            className="w-full bg-[#ff5722] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-orange-200 mt-4"
          >
            Create Account <ArrowRight size={20} />
          </button>
        </form>
        
        {/* Footer link to switch back to Login */}
        <p className="mt-10 text-center text-gray-400 text-sm font-medium">
          Already have an account? <a href="/login" className="text-[#ff5722] font-bold hover:underline transition-all">Login here</a>
        </p>
      </div>
    </AuthLayout>
  );
